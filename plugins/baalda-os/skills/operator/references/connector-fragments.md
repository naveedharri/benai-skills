...
> - After writing, `read_note` and confirm the meeting note actually contains the transcript summary, attendees, and action items. File-exists is not enough.
...
```
- **vault** ({{VAULT_MCP_NAME}}): all file I/O against vault `{{VAULT_NAME}}`, addressed by full path from the vault root. Moves and renames go through `move_note` / `move_folder`, never a shell `mv`.
