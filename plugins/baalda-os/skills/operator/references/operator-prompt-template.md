...
2. Scan before read (`search_notes` / `list_notes` before `read_note`).
3. Write only on delta. If content equals current file, skip.
4. **Verify content, not just existence.** After every write, `read_note` the path back AND confirm the new sections / data are actually present. File-exists is not enough. Retry once on mismatch or silent-fail, then log to Errors.
...
## Baalda MCP Path Convention (critical)

Baalda addresses a note by **vault plus a path from the vault root**. There is no folder-splitting, and no root-folder-name quirk: `CLAUDE.md` at the vault root is simply `/CLAUDE.md`.

The tools: `list_vaults`, `list_folders`, `list_notes`, `search_notes`, `read_note`, `create_note`, `update_note`, `append_note`, `move_note`, `move_folder`, `create_folder`.

This vault is `{{VAULT_NAME}}`. Its top-level folders:
...
- Root-level `CLAUDE.md` → `read_note` with `path: "/CLAUDE.md"`.
- `{{OPERATOR_TASK_LIST_PATH}}` → `read_note` with that path as written.
- `{{BRIEFING_PATH_PATTERN}}` → `create_note` or `update_note` with that path.

Wrong (will fail or write to the wrong place):

- Passing a bare filename with no leading slash and no folder prefix.
- Naming a vault that `list_vaults` did not return.
- Writing anywhere under `.context/`, `.git/` or `attachments/`.

If unsure whether a note exists, `list_notes` or `search_notes` first and copy the exact path string back from the response.

## Moving and renaming notes (the one that bites)

**Never `mv`, `rm`, or shell-rename a note in this vault.** A note's identity is its `doc_id`, not its path. An external move reads to Baalda as delete plus create: the note comes back with a **new** `doc_id`, and its version history, backlinks and share settings do not follow it. That damage is silent and it is not recoverable from the file alone.

- To move or rename a note → `move_note`. To move a folder → `move_folder`.
- If the Baalda MCP is not available this run, **do not move the note at all.** Append the intended move to `## Housekeeping Queue` as a task with the current path and the proposed path, and leave the file where it is.
- Creating and editing notes with ordinary writes is fine and always has been. The watcher merges the change as a CRDT operation in about 150ms, so an operator edit and a human keystroke coexist rather than clobbering each other. It is only *move* and *delete* that carry the `doc_id` risk.
- A note that reappears after a move, at zero bytes, is a sync ghost. Do not re-move it and do not copy its contents anywhere. Log it to Errors and leave it.

## Bootstrap (single parallel batch)

- `read_note` root `CLAUDE.md` at `/CLAUDE.md`.
- `read_note` `{{OPERATOR_TASK_LIST_PATH}}`.
- `list_notes` `{{BRIEFING_FOLDER}}`.
- `list_notes` `{{PROFILE_DAILY_PATH_PATTERN}}` for each team member.
...
- `read_note` the most recent existing `{{BRIEFING_PATH_PATTERN}}` (briefing template).
- `read_note` the most recent existing `{{PROFILE_DAILY_PATH_PATTERN}}{YYYY-MM-DD}.md` for each active member. If the profile has no prior daily, fall back to the root daily template.
...
- Be verified with a `read_note` round-trip after writing — content present, not just file present. On mismatch or missing sections, retry once. Still missing → log to Errors.
...
- Files in wrong folder per Knowledge Routing → flag only. Never move a synced note automatically, even with a task: queue it as a `move_note` for review.
...
- Use the full vault-root path for every Baalda MCP call. Bare filenames will fail or write to the wrong place.
- Never `mv` or `rm` a synced note. Moves and renames go through `move_note` / `move_folder`, or they become a Housekeeping Queue task. A shell move costs the note its `doc_id`, history and share settings.
- Never read, write or index `.context/`, `.git/` or `attachments/`.
- After writing to `{{BRIEFING_FOLDER}}` or any root-level folder, verify with `read_note` AND confirm content presence. On missing or mismatched content, retry once. Still failing → log to Errors.
...
Every failure logs to Errors; the run continues. Retry the write once. No other retries. All connectors failing in sequence → minimal outage report, stop.

- CLAUDE.md unreadable → use this prompt's minimal conventions. CLAUDE.md lives at `/CLAUDE.md`.
...
- write silent-fail (write succeeded but read-back missing or content not actually written) → retry once with corrected folder + leading-slash path. Log to Errors if still missing.
