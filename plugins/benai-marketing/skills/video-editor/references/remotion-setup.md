...
**Contents:** §1 Project Initialization · §2 Project Structure · §3 Workflow · §4 Common Components · §5 Animations · §6 Composition Definition · §7 Hooks · §8 Edge Cases & Gotchas · §9 Tips
...
**Note:** `@remotion/transitions` is only needed for crossfade mode. Even then, do NOT use its `TransitionSeries` component - it has a null-props bug that crashes Studio in current Remotion versions. Hard cuts (the default for social clips) use plain `<Sequence>` + `premountFor={60}`; see transitions.md for the crossfade workaround.
...
### Source Video Codec
Source videos must be H.264 with `-movflags +faststart` before they go into `public/`. AV1 (YouTube's frequent default) decodes too slowly in Studio, and without faststart the moov atom sits at the end of the file so Studio reads the whole file before it can seek. Already-H.264 files remux losslessly: `ffmpeg -i in.mp4 -c copy -movflags +faststart out.mp4`.
...
Hard-cut mode (default): total duration = sum of clip durations. No overlap, no subtraction.

Crossfade mode only - overlapping transitions reduce total duration:
```tsx
...
- Use `premountFor={60}` on every clip Sequence to preload videos - without it `OffthreadVideo` hasn't decoded the next clip's first frame and you get a black flash at hard cuts
