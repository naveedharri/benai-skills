...
**Contents:** §1 Animations · §2 Interpolate · §3 Spring Animations · §4 Video Component · §5 Sequences · §6 Common Patterns · §7 Audio Handling · §8 Captions Integration · §9 OffthreadVideo for Performance · §10 Rendering Tips
...
Always premount sequences to preload content before it appears. In hard-cut mode (the default for social clips) `premountFor={60}` is MANDATORY on every clip Sequence - without it `OffthreadVideo` hasn't decoded the next clip's first frame and you get a black flash at the cut:
...
Crossfade mode only - hard-cut mode (the social-clip default) never overlaps clips. Use negative offset in Series:
...
For text and graphic elements. Never fade clip video or audio in hard-cut mode (the social-clip default) - clips start and end at full volume on hard cuts.
...
For music beds and crossfade mode only. In hard-cut mode (the default) clip audio starts and stops at full volume; the last word is protected by per-clip `audioBufferMs`, not by fades.
...
export const CaptionPage: React.FC<CaptionPageProps> = ({
  page,
  highlightColor = '#B8E0B0', // brand kit accent - swap in the user's brand kit
  textColor = '#F5F2EA',      // brand kit light
}) => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
    ...
    >
      <div
        style={{
          fontSize: 60,
          fontWeight: 'bold',
          fontFamily: 'Inter, sans-serif', // brand kit heading font - swap in the user's
          textAlign: 'center',
          maxWidth: '80%',
...
Load the captions JSON asynchronously with `fetch(staticFile(...))` + `delayRender`/`continueRender` - never `import` it (bloats the bundle, kills hot reload). Full loading pattern in captions.md.
