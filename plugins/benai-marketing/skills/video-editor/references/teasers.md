...
**Contents:** §1 Editing mode for teasers · §2 Intelligent Teaser Workflow · §3 Teaser Clip Selection Rules · §4 Cut Quality Rules · §5 Example: Selecting Highlights · §6 Teaser Types · §7 Overlay Teaser (Picture-in-Picture) · §8 Teaser Settings · §9 Implementation · §10 FFmpeg Approach · §11 Remotion Approach · §12 Edge Cases & Gotchas

Create intelligent teaser clips using transcript-based cutting for clean, impactful highlights.

## Editing Mode for Teasers

Teasers and trailers are one of the few places crossfade mode is legitimate - blended cuts can work for a montage feel. But **hard cuts are also valid for punchy teasers and are the DEFAULT for anything published as a feed clip** (LinkedIn, TikTok, Reels). Confirm the mode in intake:

- **Hard cuts:** plain `Sequence` + `premountFor={60}`, no fades on video or audio - see [stitching.md](stitching.md). Never use `TransitionSeries` (Studio crash bug).
- **Crossfades:** follow the cosine-curve audio pattern in [transitions.md](transitions.md) so voices never overlap as noise. The short 0.4s fades used for muted overlay/montage visuals below do NOT apply to full-screen clips with spoken audio - those need the 1s visual / 1.5s audio-out / 1.2s audio-in crossfade spec.

Either way, clip boundaries come from whisper word timestamps (transcription-first workflow in [transcription.md](transcription.md)), with a per-clip `audioBufferMs` so the last word resolves.
...
- Hard-cut mode: no fades - let the last word resolve via per-clip `audioBufferMs` (whisper timestamps run 200-700ms ahead; see [transcription.md](transcription.md))
- Crossfade mode only: fade-out (200-500ms) at clip end
...
|---------|-------|---------|
| **Playback Speed** | 1.3x | Creates energy without being too fast (1.5x is too fast) |
| **Transitions** | Hard cuts (default) or fade in crossfade mode | Hard cuts for feed clips; 0.4s fades only for muted montage visuals |
| **Fade Out** | 0.5s (crossfade mode only) | Clean transition to main content |
  ...
  return (
    <Series>
      ...
      <Series.Sequence durationInFrames={3 * fps}>
        <AbsoluteFill
          style={{
            backgroundColor: '#0A0A14', // brand dark - swap in the user's brand kit
