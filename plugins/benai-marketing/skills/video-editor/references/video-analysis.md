...
If the output is NOT `h264` (YouTube often serves AV1), transcode to H.264 before using the file in Remotion - AV1 is too slow to decode in Studio. Always add `-movflags +faststart` so the moov atom sits at the front; already-H.264 files remux losslessly: `ffmpeg -i in.mp4 -c copy -movflags +faststart out.mp4`.
...
### Before Transitions (crossfade mode only - hard cuts are the default for social clips)
