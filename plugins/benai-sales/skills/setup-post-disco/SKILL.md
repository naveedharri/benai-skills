...
## Step 8b: Personalize the post-disco-followup skill in place

If the `post-disco-followup` skill is already installed (check the user's skill directories: `~/.claude/skills/post-disco-followup/`, a plugin's `skills/post-disco-followup/`, or the project's skill folder), EDIT that installed skill in place using everything the interview extracted. Do NOT create a copy of the skill, and do not leave it generic.

Personalize it so it reads as theirs:
- Bake their defaults into the skill text: their notetaker as the transcript source, their proposal backend and template, their email backend, their `process` defaults (proposal timing, first-call output, default next step), and their offer's name and framing where the skill gives examples.
- Their trigger phrases: add the company or offer names they actually use ("follow-up for the [their program] call") to the description so it fires naturally.
- Keep `config/` as the runtime source of truth. The edits set the skill's defaults and voice; config still wins at run time, so future config edits do not require re-editing the skill.

If the followup skill is NOT installed anywhere, say so and install it (or point them to the plugin) before the dry run. Setup without the engine is useless.
