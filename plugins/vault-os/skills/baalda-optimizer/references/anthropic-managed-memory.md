...
For vault analogues: a synced Baalda vault gets the same properties for free — per-note version history plus vault-wide checkpoints, both revertible. A local-only vault has neither, so git is its only rollback path. The vault audit doesn't enforce versioning (that's the sync layer's job) but flags when a vault has *no* rollback path at all.
...
- **Versioning**: flag only if the vault has no rollback path — i.e. it is local-only (no `.context/config.json`, so no server-side note history or checkpoints) *and* not under git. No versioning = no audit trail = no rollback.
