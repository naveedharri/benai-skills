...
**Framework rule:** memory writes produce immutable named versions; the vault analogue is a rollback path.

**Trigger heuristic (Baalda):**
- `.context/config.json` exists → the vault is synced, so it *already has* per-note version history
  and vault-wide checkpoints. **Not** a finding.
- `.context/` exists but no `config.json` → local-only vault. Per-note history and checkpoints are
  server-side, so this vault has no rollback path unless `.git/` exists.
...
- Check `.gitignore` if `.git/` exists — is the vault actually being committed (not entirely ignored)?
  It should ignore `.context/` and commit the notes; the reverse is a finding of its own.
...
**Finding format:**
```
Local-only vault has no rollback path
Reasoning: {N} files of substantive content with no audit trail; bulk operations are irreversible
Action: turn on sync for this vault (Vault settings → General) to get per-note history and
        checkpoints, OR `git init` and commit
