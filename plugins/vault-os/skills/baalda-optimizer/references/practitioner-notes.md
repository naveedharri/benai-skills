...
- **No CLI needed in Baalda.** Mark's setup leaned on the Obsidian CLI's 95 commands; here the equivalent is plain file access plus the MCP tools for anything structural (move/rename/delete).
...
- **Canvas** — Obsidian has a native canvas. Baalda has a graph view derived from wikilinks instead, so the equivalent lever is writing better links, not populating a canvas.
...
├── Projects/{name}/
└── .context/              # Baalda's index + CRDT store (never touch, never commit)
```

Claude Code points at the vault root and edits the `.md` files directly — in Baalda that is the whole integration, since the files *are* the source of truth.
