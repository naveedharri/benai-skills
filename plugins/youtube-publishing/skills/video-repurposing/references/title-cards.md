...
**Contents:** §1 Types of Title Cards · §2 ALWAYS ASK User Preferences · §3 Brand-Aware Title Cards · §4 Placement: Before Video · §5 Placement: Overlay at Start · §6 Lower Third Style · §7 Animation Patterns · §8 Common Patterns · §9 Edge Cases & Gotchas
...
Title cards use the brand kit collected in intake (see SKILL.md). Keep it in one constant:
...
const BRAND = {
  ...
  accent: '#B8E0B0',  // accent lines, subtitles, highlights
...
export const TitleCard: React.FC<TitleCardProps> = ({
  ...
  subtitle,
  // example values - swap in the user's brand kit (see SKILL.md)
  colors = {primary: '#0A0A14', secondary: '#B8E0B0', text: '#ffffff'},
}) => {
    ...
    <AbsoluteFill>
      ...
      <Sequence from={0} durationInFrames={TITLE_CARD_DURATION}>
        <TitleCard
          ...
          colors={{
            primary: 'rgba(10, 10, 20, 0.7)', // Semi-transparent brand dark
            secondary: '#B8E0B0',              // accent (example value)
            text: '#ffffff',
          }}
        />
      </Sequence>
      ...
      >
        ...
        <div
          style={{
            width: 4,
            height: 60,
            backgroundColor: '#B8E0B0', // accent (example value - swap in the user's brand kit)
            position: 'absolute',
            left: -20,
            top: 0,
          }}
...
If using custom fonts (Inter, or whatever the brand kit specifies), ensure they're loaded:
```tsx
...
For overlay title cards where video is visible behind (use the brand dark, not pure black):
```tsx
backgroundColor: 'rgba(10, 10, 20, 0.7)', // 70% opacity brand dark
```
