...
- **The deal-file structure.** One file per prospect, named by the prospect. Tracking freezes the moment a deal is won or lost, because this is sales, not fulfilment. Frontmatter: status (mirrors the CRM stage), prospect, company, email, phone, deal size, source, plus the call-prep links the dashboard surfaces per call: linkedin, website, meeting_url (omit any link you do not have). Sections: a snapshot, a history with newest at the bottom, the next step. Trust the proposal platform's status over a stale CRM stage. The morning routine writes the links; the dashboard reads them onto each call card and its per-prospect sub-page.
