...
**Contents:** §1 Types of Video Graphics · §2 ALWAYS ASK User Preferences · §3 Remotion Graphics (React-Based) · §4 AI Image Generation · §5 Combining AI + Remotion · §6 Template Library · §7 Batch Generation Script · §8 AI Prompt Best Practices · §9 Edge Cases & Gotchas
...
const BRAND = {
  dark: '#0A0A14',    // backgrounds - never pure #000 black, always the brand dark
  light: '#F5F2EA',   // light tone
  accent: '#B8E0B0',  // accent lines, subtitles
  font: 'Inter',      // heading font via @remotion/google-fonts
};
...
// Example: Generate a thumbnail background (describe the brand-kit colors in the prompt)
const backgroundUrl = await generateImage(
  'Abstract geometric background with deep dark blue and soft green accent gradients, modern tech aesthetic, suitable for video thumbnail'
);
```
  ...
  return (
    <AbsoluteFill>
      ...
      >
        ...
        {subtitle && (
          <p style={{color: '#B8E0B0', fontSize: 28, marginTop: 16}}> {/* accent - example value */}
            {subtitle}
          </p>
        )}
      </AbsoluteFill>
...
export const QuoteCard: React.FC<{
  ...
  author: string;
  backgroundColor?: string;
}> = ({quote, author, backgroundColor = '#0A0A14'}) => { // brand dark (example value)
  return (
    <AbsoluteFill
      style={{
        backgroundColor,
    ...
    >
      {/* Quote marks */}
      <span style={{fontSize: 120, color: '#B8E0B0', opacity: 0.3}}>"</span> {/* accent - example value */}
      ...
      <p style={{color: '#B8E0B0', fontSize: 28, marginTop: 40}}> {/* accent - example value */}
        &mdash; {author}
      </p>
    </AbsoluteFill>
  );
...
export const EndScreen: React.FC<{
  ...
  return (
    <AbsoluteFill
      style={{
        backgroundColor: '#0A0A14', // brand dark - never pure #000 (example value)
        display: 'flex',
        flexDirection: 'column',
        justifyContent: 'center',
        alignItems: 'center',
    ...
    >
      ...
      <div
        style={{
          ...
          backgroundColor: '#B8E0B0', // accent (example value)
          padding: '20px 60px',
          borderRadius: 40,
        }}
      >
        <span style={{color: '#0A0A14', fontSize: 32, fontWeight: 600}}>
          {ctaText}
        </span>
      </div>
    </AbsoluteFill>
...
```
✅ Good: "Abstract geometric background with deep dark blue (#0A0A14) and soft green accent (#B8E0B0) gradients, modern tech aesthetic, subtle particle effects, 16:9 aspect ratio, suitable for video thumbnail"

❌ Bad: "cool background"
```

Use the actual hex values from the user's brand kit in the prompt - AI image models follow named hex codes surprisingly well.

### For Thumbnails

```
✅ Good: "Professional YouTube thumbnail background, dramatic lighting, dark moody atmosphere with accent lighting in the brand accent color (#B8E0B0), no text, space for overlay text on left side"
...
Ensure fonts are loaded before rendering (use the brand kit's heading font):
```tsx
