...
**Contents:** §1 Project Structure · §2 package.json · §3 Root.tsx · §4 FinalVideo.tsx · §5 Whisper Setup · §6 Fast Transcription Script · §7 Caption JSON Format · §8 Setup Commands · §9 Customization Checklist · §10 Summary
...
// Brand kit - example values, swap in the user's brand kit (see SKILL.md)
const BRAND = {
  dark: '#0A0A14',    // backgrounds + pill fill - never pure #000 black
  light: '#F5F2EA',   // spoken words
  accent: '#B8E0B0',  // active word, pill border
  font: inter,        // heading font, loaded above via @remotion/google-fonts
};
...
// Caption settings
const SWITCH_CAPTIONS_EVERY_MS = 3500; // More words per page, less frequent swapping
const CAPTION_COLORS = {
  active: BRAND.accent,    // currently speaking word
  spoken: BRAND.light,     // already spoken words
  upcoming: '#EAF2F5',     // second light tone (subtle) - not yet spoken
};
// Frosted pill (the skill's default caption style - see references/captions.md)
const CAPTION_PILL = {
  bg: 'rgba(10, 10, 20, 0.55)',              // brand dark at 0.55 alpha
  border: 'rgba(184, 224, 176, 0.3)',        // accent at 0.3 alpha (subtle)
...
function createVolumeFn(
  ...
  return (f: number) => {
    ...
    if (!isFirst && f < AUDIO_FADEIN_FRAMES) {
      ...
      vol = (1 - Math.cos(progress * Math.PI)) / 2; // Cosine ease: 0 → 1
    }
    ...
    if (!isLast) {
      ...
      if (f >= fadeOutStart) {
        ...
        vol *= (1 + Math.cos(progress * Math.PI)) / 2; // Cosine ease: 1 → 0
      }
    }
...
const CaptionPage: React.FC<{page: TikTokPage; pageStartMs: number}> = ({page, pageStartMs}) => {
  const frame = useCurrentFrame();
  const {fps, durationInFrames} = useVideoConfig();

  // frame is local to this Sequence (starts at 0), so add pageStartMs
  // to get the absolute clip time for token comparison
  const currentTimeMs = pageStartMs + (frame / fps) * 1000;

  // Cap fade to avoid inputRange monotonic error on short captions
  const maxFade = Math.floor(durationInFrames / 3);
  const fadeFrames = Math.min(Math.round(0.2 * fps), maxFade);

  const captionOpacity = fadeFrames > 0 && durationInFrames > 2
    ? interpolate(
        frame,
  ...
  return (
    <AbsoluteFill
      style={{
        ...
        alignItems: 'center',
        paddingBottom: 30, // low enough that the speaker's face/gestures stay unobstructed
        opacity: captionOpacity,
      }}
    >
      <div
        style={{
          backgroundColor: CAPTION_PILL.bg,
          border: `2px solid ${CAPTION_PILL.border}`,
          borderRadius: 12,
          boxShadow: 'none', // the frosted blur replaces the solid drop shadow
          backdropFilter: CAPTION_PILL.backdropFilter,
          WebkitBackdropFilter: CAPTION_PILL.backdropFilter,
          padding: '8px 18px', // tight - keeps the pill compact
          maxWidth: '80%',
        }}
      >
        <div
          style={{
            fontSize: 28, // deliberately small - the focus is the speaker, not the captions
            fontWeight: 700,
            fontFamily: `${BRAND.font}, Georgia, sans-serif`,
            textAlign: 'center',
            lineHeight: 1.5,
        ...
        >
          {page.tokens.map((token, i) => {
            ...
            const isSpoken = token.toMs <= currentTimeMs;
...
npm install remotion @remotion/cli @remotion/captions @remotion/transitions @remotion/bundler @remotion/install-whisper-cpp react react-dom
...
3. **Adjust settings** if needed:
   ...
   - `BRAND` / `CAPTION_COLORS`: swap in the user's brand kit (accent = active word, light = spoken, second light tone = upcoming)
...
- **Transcription**: Word-level timestamps via Remotion `transcribe()` API with `tokenLevelTimestamps: true`
- **Teaser**: 1.3x speed with fade transitions
- **Captions**: frosted-pill word highlighting (accent = active, light = spoken, second light tone = upcoming) with 0.2s fade
