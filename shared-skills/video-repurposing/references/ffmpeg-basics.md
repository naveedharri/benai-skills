...
## Source Video Prep for Remotion (H.264 + faststart)

**H.264 only.** AV1 is too slow to decode in Remotion Studio - previews stutter and crawl. YouTube often serves AV1 by default, so always check the codec of a downloaded video before dropping it into `public/`:

```bash
ffprobe -v error -select_streams v:0 -show_entries stream=codec_name -of csv=p=0 video.mp4
```

If it's `av1` (or `hevc`), transcode to H.264:

```bash
ffmpeg -i source.mp4 -c:v libx264 -crf 23 -preset fast -c:a aac -b:a 192k -movflags +faststart output_h264.mp4
```

**`-movflags +faststart` is mandatory.** Without it the moov atom (the index of the file) sits at the END of the file, and Remotion must read the entire file before it can seek to any frame. Symptom: Studio takes forever to load the video. If you already have an H.264 file without faststart, remux losslessly - no re-encode needed:
...
**Note:** fades are for crossfade mode only (trailers/teasers/montages). Social clips (LinkedIn/TikTok/Reels) use hard cuts with no fades of any kind - see [transitions.md](transitions.md).
