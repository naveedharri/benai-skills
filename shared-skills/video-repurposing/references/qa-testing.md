...
| Black flash at a hard cut | Missing `premountFor` on the Sequence | Add `premountFor={60}` to every clip Sequence |
| Studio crash on transitions | `TransitionSeries` null-props bug | Use plain `Sequence` hard cuts; crossfade workaround in transitions.md |

### 5. Editing-Mode Checks

**Hard-cut mode (the default for social clips):**

- [ ] No black flash at any cut - every clip `<Sequence>` has `premountFor={60}`
- [ ] Last word of each clip resolves fully by ear - per-clip `audioBufferMs` is correct (whisper timestamps run ~200-700ms ahead of actual speech)
- [ ] No next-sentence caption bleed - captions are filtered by the raw transcript `endMs`, not the buffered audio end
- [ ] No fades or crossfades anywhere - audio starts and stops at full volume

**Crossfade mode only (when the user explicitly picked it):**

- [ ] Transitions are smooth - cosine volume curves, no overlapping audio noise
- [ ] No fade-in on the first clip, no fade-out on the last
- [ ] Built with the plain-Sequence crossfade workaround, NOT `TransitionSeries` (null-props bug crashes Studio)
