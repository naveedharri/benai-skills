---
name: image
description: Generate on-brand images and infographics for LinkedIn posts and newsletters using Nano Banana MCP (Gemini AI image generation). Final step of the content pipeline. Follows a guided flow. Analyze the content, suggest visualizable concepts, propose visualization approaches, then generate on-brand images. USE THIS SKILL WHEN user says "create an image", "make a visual", "generate an infographic", or wants a graphic to go with a post or newsletter. Modular architecture with lazy-loaded reference files for each phase.
---
